...
void *not_cached_in_core(){
    set_affinity(pthread_self(),core_id);
    non_cached_var = rand()%100+non_cached_var%10;
    cached_var += measure_flush_latency(&non_cached_var);
    pthread_mutex_lock(&mutex);
    thread_running = 1;
    pthread_cond_signal(&cond);
    pthread_mutex_unlock(&mutex);
...
int attacker(void* addr, int cached)
{
    ...
    if(core_id==-1 && cached){
        ...
        compute_intensive_loop(400);
        // pthread_create(&t[0],NULL,not_cached_in_core,NULL);
        // pthread_mutex_lock(&mutex);
        // while (thread_running == 0) pthread_cond_wait(&cond, &mutex);
    }
