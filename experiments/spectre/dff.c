...
#define T_ARRAY_SIZE (3)

/* cache hit threshold for F+F */
int cache_hit_threshold = 240;
...
uint64_t tl_array[T_ARRAY_SIZE] = {250, 156, 100};
uint64_t th_array[T_ARRAY_SIZE] = {300, 239, 118};
...
uint64_t measure_access_latency(volatile uint8_t* addr, unsigned int junk){
    ...
    _mm_lfence();
    _mm_clflush(&addr); /* MEMORY ACCESS TO TIME */
    _mm_mfence();
    _mm_lfence();
